...
const Visualres = () => {
  const location = useLocation();
  const [data, setData] = useState(null);
  console.log('Component rendered. Location state:', location.state);

  useEffect(() => {
    console.log('useEffect triggered. Location state:', location.state);
    
    if (location.state?.serverData?.review_summary) {
      const reviewData = location.state.serverData.review_summary;
      console.log('Review data before setting:', reviewData);
      
      // Only set data if it's different from current data
      setData(prevData => {
        if (JSON.stringify(prevData) !== JSON.stringify(reviewData)) {
          console.log('Setting new data');
          return reviewData;
        }
        console.log('Data unchanged, keeping previous state');
        return prevData;
      });
    }
  }, [location.state]);
  ...
  if (!data || !data.barChartData || !data.pieChartData) {
    console.log('Data not ready yet:', data);
